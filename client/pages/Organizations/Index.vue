...
throttledWatch(
  form,
  () => {
    Inertia.get('organizations', form.value, { preserveState: true })
  },
...
<template>
  <div>
    ...
    <header>
      ...
      >
        <h1 class="text-3xl font-bold leading-tight text-gray-800">Organizations</h1>
      </div>
      <div
      ...
      <div class="max-w-screen-2xl mx-auto sm:px-6 lg:px-8">
        <div class="w-full mt-6 shadow border-gray-200 sm:rounded-lg md:overflow-hidden">
          <div class="md:flex-1 overflow-x-auto">
            <Table
              :headers="headers"
              :rows="organizations.data.map(item => ({ ...item, link: `/organizations/${item.id}` }))"
            >
              <template #name="{ row }">
